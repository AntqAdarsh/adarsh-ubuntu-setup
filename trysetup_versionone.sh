...
sudo apt-get install -y curl wget git software-properties-common apt-transport-https ca-certificates gnupg lsb-release expect cups
...
sudo wget https://github.com/rustdesk/rustdesk/releases/download/1.2.6/rustdesk-1.2.6-x86_64.deb
sudo apt install -fy ./rustdesk-1.2.6-x86_64.deb
...
header "Installing HP Plugin via Expect"
sudo -u "$SUDO_USER" expect <<EOF
spawn hp-plugin -i
expect "Do you accept the license agreement*"
send "y\r"
expect "Download and install the plug-in*"
send "d\r"
expect eof
EOF
if [ $? -eq 0 ]; then
...
header "Running HP Setup via Expect"
sudo -u "$SUDO_USER" expect <<EOF
spawn hp-setup -i
expect {
...
echo "Test Page from Adarsh Setup Script" > /tmp/testprint.txt
lp /tmp/testprint.txt && log_success "Test Print Sent" || log_failure "Test Print Failed"
